  ...
  //const update = useScan;
    ...
    const handleScan = (data) => {
        if(!data.startsWith("w") || data.split(" ").length !== 3) {
            ...
            //fetch(`http://10.30.30.13:8000/freight_quote`, { method: 'PUT', body: newScan })
            fetch(configData.mode === "production" ? configData.apiUrl + `/freight_quote` : `/freight_quote`, { method: 'PUT', body: newScan })
                .then(handleErrors)
